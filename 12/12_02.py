...
def find_path(starting_path):
    for connection in dic_of_nodes[starting_path[-1]]:
        ...
        if connection.islower():
            counter = collections.Counter([ch for ch in starting_path if ch.islower()])
            if connection == "start":
                pass
            elif connection not in starting_path:
                new_list = starting_path.copy()
                new_list.append(connection)
                find_path(new_list)
            elif starting_path.count(connection) < 2 and max(counter.values()) < 2:
                new_list = starting_path.copy()
                new_list.append(connection)
                find_path(new_list)
            else:
