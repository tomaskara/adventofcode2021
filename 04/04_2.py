    ...
    for index_of_board, board in enumerate(boards):
        for index_of_item, item in np.ndenumerate(board):
            if item == num:
                ...
                if (np.all(marked_boards[index_of_board] >= 100, axis=1)[r] or
                    np.all(marked_boards[index_of_board] >= 100, axis=0)[c]) and \
                        len(winner_boards) == len(marked_boards)-1 and index_of_board not in winner_boards:
                    sum_ummarked = 0
                    ...
                    quit()
                elif np.all(marked_boards[index_of_board] >= 100, axis=1)[r] or \
                        np.all(marked_boards[index_of_board] >= 100, axis=0)[c]:
                    winner_boards.add(index_of_board)
                    sum_ummarked = 0
                    for _, element in np.ndenumerate(marked_boards[index_of_board]):
                        if element < 100:
                            sum_ummarked += element
